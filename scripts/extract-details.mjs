...
function extract(text, fileName) {
  ...
  for (const t of variants(text)) {
    ...
    // למשל בחשבוניות כביש 6: "925002279731 חשבונית מס:")
    if (!out.docNumber) {
      const m = t.match(/(?:חשבונית(?:\s*מס)?(?:\s*\/?\s*קבלה)?|חשבון\s*[\/-]?\s*קבלה|קבלה)\s*(?:מספר|מס'?|[#:])?\s*(\d{3,12})/)
        ...
        || t.match(/(\d{3,15})\s*(?:חשבונית(?:\s*מס)?(?:\s*\/?\s*קבלה)?|קבלה)\s*[:#]/)
        || t.match(/#\s*(\d{3,12})/);
      if (m) out.docNumber = m[1];
    }
    // גיבוי: מספר ארוך בשם הקובץ (הורדות של חשבוניות דיגיטליות נשמרות עם המספר)
    if (!out.docNumber) {
      const fm = fileName.match(/_(\d{9,15})(?:\.\d+)?\.pdf$/i);
      if (fm) out.docNumber = fm[1];
    }
