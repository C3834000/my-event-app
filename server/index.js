...
function invokeNetlifyHandler(relativePath) {
  const url = pathToFileURL(path.join(__dirname, '..', 'netlify', 'functions', relativePath)).href;
  return import(url).then((mod) => mod.handler);
}

async function proxyNetlifyGet(relativePath, req, res) {
  try {
